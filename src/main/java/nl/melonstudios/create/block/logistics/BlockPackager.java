...
/**
 * Packager: pulls requested items from the attached inventory (below) into
 * packages. Right-click with empty hand: package everything available.
 * Stock ticker (companion block) places filtered requests.
 */
...
public class BlockPackager extends BlockKineticBase implements ITileEntityProvider {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (te instanceof TileEntityPackager) {
            ((TileEntityPackager) te).packageAll();
            player.sendStatusMessage(new net.minecraft.util.text.TextComponentString("Packaging requested items..."), true);
        }
        return true;
    }
